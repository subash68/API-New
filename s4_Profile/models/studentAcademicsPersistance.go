...
func getAllSemDetails(ID string) []StudentSemesterModel {
	...
	for semRows.Next() {
		...
		err = semRows.Scan(&newSem.ID, &newSem.StudentStakeholderID, &newSem.UniversityStakeholderID, &newSem.IsGrad, &newSem.IsPG, &newSem.Semester, &newSem.StudentCollegeRollNo, &newSem.ProgramName, &newSem.ProgramID, &newSem.BranchName, &newSem.BranchID, &newSem.CGPA, &newSem.Percentage, &newSem.AttachFile, &newSem.AttachmentName, &newSem.EnabledFlag, &newSem.CreationDate, &newSem.LastUpdatedDate, &newSem.SemesterCompletionDate, &newSem.SentforVerification, &newSem.DateSentforVerification, &newSem.Verified, &newSem.DateVerified, &newSem.SentbackforRevalidation, &newSem.DateSentBackForRevalidation, &newSem.ValidatorRemarks, &newSem.VerificationType, &newSem.VerifiedByStakeholderID, &newSem.VerifiedByEmailID)
		if err != nil {
...
func verifyAttachment(binaryfile string, fileName string) (err error) {
	if binaryfile != "" {
		...
		if fileName == "" {
			return fmt.Errorf("Attachment File name 'attachFileName' is required")
		}
	} else {
