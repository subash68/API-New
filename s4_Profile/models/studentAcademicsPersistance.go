...
func GetAcademics(ID string) (StudentAcademicsModelReq, error) {
	...
	err := Db.QueryRow(getByIDSP, ID).Scan(&tenN.Name, &tenN.Location, &tenN.MonthAndYearOfPassing, &tenN.Board, &tenN.Percentage, &tenN.AttachmentFile, &tenN.EnablingFlag, &twelfthN.Name, &twelfthN.Location, &twelfthN.MonthAndYearOfPassing, &twelfthN.Board, &twelfthN.Percentage, &twelfthN.AttachmentFile, &twelfthN.EnablingFlag)
	if err != nil {
