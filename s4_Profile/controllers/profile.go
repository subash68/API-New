...
func UpdateProfile(c *gin.Context) {
	...
	dbError := models.UpdateProfileData(updateReq, sp, "PROFILE_UPDATE_WHERE", ID, attachmentUpdate, attachment, ppUpdate, ppfile)
	if dbError.ErrTyp != "000" {
		resp := ErrCheck(ctx, dbError)
		c.Error(dbError.Err)
		c.JSON(http.StatusInternalServerError, resp)
