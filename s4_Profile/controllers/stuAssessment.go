...
func AddAssessment(c *gin.Context) {
	ctx, ID, _, successResp := getFuncReq(c, "Add Assessment")
	binding.Validator = &defaultValidator{}
	var sa models.StudentAssessmentModel
	...
	if err == nil {
		...
		c.JSON(http.StatusOK, models.MessageResp{"Assessment Saved"})
	}
...
func GetAllAssessments(c *gin.Context) {
	ctx, ID, _, successResp := getFuncReq(c, "Get Assessments")
	binding.Validator = &defaultValidator{}
	var sa models.StudentAllAssessmentModel
	err := c.ShouldBindWith(&sa, binding.Form)
	if err == nil {
		sa.StakeholderID = ID
		err := sa.GetAllAssessment()
		if err != nil {
			resp := ErrCheck(ctx, models.DbModelError{ErrCode: "S3PJ", ErrTyp: "Failed to Process request", Err: err, SuccessResp: successResp})
			c.JSON(http.StatusInternalServerError, resp)
			c.Abort()
		...
		c.JSON(http.StatusOK, sa.Assessments)
		c.Abort()
		return
	}
	resp := ErrCheck(ctx, models.DbModelError{ErrCode: "S3PJ", ErrTyp: "Required information not found", Err: err, SuccessResp: successResp})
...
func UpdateAssessment(c *gin.Context) {
	ctx, ID, _, successResp := getFuncReq(c, "Update Assessments")
	binding.Validator = &defaultValidator{}
	var sa models.StudentAssessmentModel
	err := c.ShouldBindWith(&sa, binding.Form)
	if err == nil {
		...
		c.JSON(http.StatusOK, models.MessageResp{"Updated Assessment"})
	}
	resp := ErrCheck(ctx, models.DbModelError{ErrCode: "S3PJ", ErrTyp: "Required information not found", Err: err, SuccessResp: successResp})
	c.JSON(http.StatusUnprocessableEntity, resp)
	return
...
func DeleteAssessment(c *gin.Context) {
	...
	c.JSON(http.StatusOK, models.MessageResp{"Deleted Assessment"})
	c.Abort()
	return
}
