...
func getFuncReq(c *gin.Context, ctxKey string) (context.Context, string, string, map[string]string) {
	...
	if !ok {
		resp := ErrCheck(ctx, models.DbModelError{ErrCode: "S7PG", ErrTyp: "Invalid information", Err: fmt.Errorf("Cannot decode User ID from the request"), SuccessResp: successResp})
	...
	if !ok {
		resp := ErrCheck(ctx, models.DbModelError{ErrCode: "s7PG", ErrTyp: "Invalid information", Err: fmt.Errorf("Cannot decode User Type from the request"), SuccessResp: successResp})
		c.JSON(http.StatusUnprocessableEntity, resp)
		c.Abort()
		return ctx, "", "", successResp
	}
