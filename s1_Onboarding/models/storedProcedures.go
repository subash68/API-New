...
func CreateSP() {
	...
	SP = map[string]string{
		...
		"STU_INS_NEW_USR":        "INSERT INTO " + dbConfig.DbDatabaseName + "." + dbConfig.StuMasterDbName + " (Stakeholder_ID,Student_FirstName,Student_MiddleName,Student_LastName,Student_PersonalEmailID,Student_PhoneNumber,Student_AlternateContactNumber,Student_Gender,Student_DateOfBirth,Student_AadharNumber,StudentPermanantAddress_Line1,StudentPermanantAddress_Line2,StudentPermanantAddress_Line3,StudentPermanantAddress_Country,StudentPermanantAddress_State,StudentPermanantAddress_City,StudentPermanantAddress_District,StudentPermanantAddress_Zipcode,StudentPermanantAddress_Phone,StudentPresentAddress_Line1,StudentPresentAddress_Line2,StudentPresentAddress_Line3,StudentPresentAddress_Country,StudentPresentAddress_State,StudentPresentAddress_City,StudentPresentAddress_District,StudentPresentAddress_Zipcode,StudentPresentAddress_Phone,University_Name,University_Stakeholder_ID,ProgramName,Program_ID,BranchName,Branch_ID,Student_CollegeID,Student_CollegeEmailID,UserPassword,UniversityApprovedFlag,CreationDate,LastUpdatedDate,AccountStatus,Primary_Phone_Verified,Primary_Email_Verified,AccountExpiryDate,Attachment,DateOfJoining)  VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
		...
		"REG_SH_COUNT":           "SELECT (SELECT COUNT(*) FROM " + dbConfig.DbDatabaseName + "." + dbConfig.CorpMasterDbName + ") as CorporatesRegistered,(SELECT COUNT(*) FROM " + dbConfig.DbDatabaseName + "." + dbConfig.StuMasterDbName + ") as StudentsRegistered,(SELECT COUNT(*) FROM " + dbConfig.DbDatabaseName + "." + dbConfig.UnvMasterDbName + ") as UniversitiesRegistered;",
	}
}
