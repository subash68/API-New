...
func CreateSP() {
	...
	SP = map[string]string{
		...
		"STU_VRF_ME_QRY":         "SELECT Student_PhoneNumber,Student_CollegeEmailID,IF(COUNT(*),'true','false') FROM " + dbConfig.DbDatabaseName + "." + dbConfig.StuMasterDbName + " WHERE Stakeholder_ID= ? GROUP BY Student_PhoneNumber,Student_CollegeEmailID LIMIT 1",
		"STU_GET_PID":            "SELECT Stakeholder_ID,IF(COUNT(*),'true','false') FROM " + dbConfig.DbDatabaseName + "." + dbConfig.StuMasterDbName + " WHERE Student_PhoneNumber= ? OR Student_CollegeEmailID= ? GROUP BY Stakeholder_ID",
		"STU_LOGIN":              "SELECT Stakeholder_ID,AccountStatus,UserPassword,IF(COUNT(*),'true','false') FROM " + dbConfig.DbDatabaseName + "." + dbConfig.StuMasterDbName + " WHERE Student_CollegeEmailID= ? OR Stakeholder_ID= ? OR Student_PhoneNumber= ? GROUP BY Stakeholder_ID,AccountStatus,UserPassword LIMIT 1",
